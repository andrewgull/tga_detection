...
cn_bins <- read_delim(file, show_col_types = F) %>% 
  rename("reads_counts_theor" = n.reads)
...
bla_cn_freq <- 
  bla_cn_freq %>% 
  mutate(reads_freq_obs = reads_counts_obs / total)

# find frequency of reads that might contain certain CN
cn_bins <- 
  cn_bins %>% 
  mutate(reads_freq_theor = reads_counts_theor / total)

# adjust CN frequency
bla_cn_freq <- 
  bla_cn_freq %>% 
  left_join(cn_bins, by = "CN") %>% 
  mutate(reads_counts_adj = reads_counts_obs / reads_freq_theor,
         reads_freq_adj = reads_counts_adj/sum(reads_counts_adj))
...
```{r}
bla_cn_freq %>% 
  select(CN, reads_freq_obs, reads_freq_adj) %>% 
