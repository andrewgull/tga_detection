...
```{r}
file_path <- "../results/reads/76595_D_mh/reads_all.fasta"
# Read the file
...
## Filtering 3: FR + IS + 1820 towards the repeats
...
red_repunit_dir_dist_len <-
  red_repunit_dir_dist %>%
  mutate(keep = if_else(
    orientation.x == "direct",
    end.subject.x > 1820,
    start.subject.y > 1820
  )) %>%
  filter(keep)
