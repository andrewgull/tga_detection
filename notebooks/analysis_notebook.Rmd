...
blastn -query resources/flanking_regions/flanking_region_red.fa -subject results/reads_filtered/replicate_e/CNV_reads_all.fasta -outfmt 6 -num_alignments 100000 -num_threads 10 > results/tables/blast_flanking_region_red.tsv

blastn -query resources/flanking_regions/flanking_region_green.fa -subject results/reads_filtered/replicate_e/CNV_reads_all.fasta -outfmt 6 -num_alignments 100000 -num_threads 10 > results/tables/blast_flanking_region_green.tsv
```

## Parse BLAST results

```{r}
blast_red <- read_delim("../results/tables/blast_flanking_region_red.tsv", col_names = F, col_select = c(X1, X2, X3, X4, X9, X10))

names(blast_red) <- c("query", "subject", "identity", "length", "start.subject", "end.subject")

blast_red$region <- "fr_red"

parse_blast_df(blast_red)
```

```{r}
# FIND CNV
n_copies <- parse_blast_df(blast_red) %>% 
  filter(length < 250) %>% 
  transmute(n.copies = round(abs(start.subject - end.subject) / 3500)) 
