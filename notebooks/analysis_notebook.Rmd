...
Threshold can easily be at 500 bp
...
Apply the same filtering here, except shortest distance
...
Use `full_join()` to retain all combinations of reads
...
There are reads containing only 'red', only 'green' and both, and thise that contain both 'red' and 'green' on the same strand.
...
blast_joined %>% filter(is.na(query.x)) %>% nrow()
```

2. Have 'red' but not 'green'

```{r}
blast_joined %>% filter(is.na(query.y)) %>% nrow()
```

3. Have both 'red' and 'green'

```{r}
blast_joined %>% filter(!is.na(query.y), !is.na(query.x)) %>% nrow()
```
...
n_copies <- blast_joined %>% 
  filter(!is.na(region.y), 
         !is.na(region.x)) %>% 
  mutate(n.copies = round(abs(start.subject - end.subject) / 3500)) 
...
```{r}
ggplot(blast_red, aes(start.subject)) +
  geom_histogram(bins = 250)
```

```{r}
summary(blast_red$start.subject)
```
...
### Length vs Identity vs Start

```{r}
ggplot(blast_red, aes(start.subject, identity)) +
