...
# trim adapters
# 1. it runs out of RAM
# 2. it's no assembly, we can skip this step
...
left_join(blaSHV_mean_len, blaSHV_counts, by = "subject") %>% 
  ...
  ggplot(aes(n.blaSHV, mean.length)) +
  geom_jitter(alpha=0.3, size = 0.5, height = 0.05)
```

**NB**: for n.blaSHV = 0 mean.length doesn't have any sense
...
```{r}
blast_joined_filt %>%
  mutate(dist.expected = n.blaSHV*len_bla + 2*FR + IS) %>% 
  ggplot(aes(green.red.distance.pos, n.blaSHV)) +
  geom_point(alpha = 0.3, color = "steelblue") +
  geom_vline(aes(xintercept = dist.expected - dist_marg*dist.expected)) +
  geom_vline(aes(xintercept = dist.expected + dist_marg*dist.expected, color=dist.expected), linetype="dotdash")
...
blast_joined_filt %>%
  mutate(dist.expected = n.blaSHV*len_bla + 2*FR + IS) %>% 
  ggplot(aes(green.red.distance.pos, dist.expected)) +
  geom_point(alpha = 0.3, color = "steelblue")
```
...
blast_joined_filt %>%
  mutate(dist.expected = n.blaSHV*len_bla + 2*FR + IS) %>% 
  ggplot(aes(dist.expected, dist.expected)) +
  geom_point(alpha = 0.3, color = "steelblue")
```
