...
# histogram
seqkit watch results/reads_filtered/replicate_e/CNV_reads_all_3.7k.fastq.gz -O results/reads_stat/replicate_e_min_3.7k.png -j 12
...
blast_joined_filt %>% 
  mutate(green.red.distance.2 = if_else(green.red.distance < 0, green.red.distance * -1, green.red.distance * 1)) %>% 
  ggplot(aes(green.red.distance.2, read.len)) +
...
idea: if I see 3 reads with 10 copies of blaSHV, it doesn't mean that there *indeed* are only three molecules with 10 blaSHV, because the chance to get long reads is lower than to get short reads - i.e it is very likely that there were more molecules but they got fragmented and were discarded from our analysis.
