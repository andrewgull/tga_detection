...
bla_bed <- 
  ...
  dplyr::rename("start" = end,
         "end" = start) %>% 
  select(chrom, start, end, name, score, strand) %>% 
  bind_rows(bla_bed %>% filter(strand == '+'))
...
sort -k1,1 -k2,2n blaSHV.bed > blaSHV_sorted.bed 

# try distance = 10
bedtools cluster -i blaSHV_sorted.bed -s -d 10 > blaSHV_clusters_d10.bed
```

Look at the results of clusters 10

```{r}
bla_clust <- read_tsv("/home/andrei/Data/nano-cnv/notebooks/bedfiles/blaSHV_clusters_d10.bed", col_names = F)
```
...
```{bash, eval=F}
distances=("10" "50" "100" "150" "200" "300" "500" "800")
