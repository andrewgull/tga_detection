...
```{bash, eval=FALSE}
# requires a lot of RAM unless you build a DB first
...
## Overall counting of blaSHV genes

Let's count all blaSHVs that we can find in the reads. That will give us an idea of how many blaSHV there is.

```{r}
# the blast file already exists
blast_blaSHV <- read_tsv("../results/tables/76595_E_mh/blast_blaSHV.tsv", show_col_types = F, col_names = F)
names(blast_blaSHV) <- c("query", "subject", "identity", "length", "mismatch",
                        "gaps", "start.query", "end.query", "start.subject",
                        "end.subject", "e.value", "bit.score")

write_delim(blast_blaSHV, file = "../results/tables/76595_E_mh/blast_blaSHV_headers.tsv", delim = "\t")
...
```{bash}
# activate rscripts-env
Rscript workflow/scripts/make_bed.R -i results/tables/76595_E_mh/blast_blaSHV_headers.tsv -o results/tables/76595_E_mh/blaSHV_hits_non_filtered.bed

# cluster hits
# activate varcalling-env
sort -k1,1 -k2,2n results/tables/76595_E_mh/blaSHV_hits_non_filtered.bed > results/tables/76595_E_mh/blaSHV_hits_non_filtered_sorted.bed && bedtools merge -i results/tables/76595_E_mh/blaSHV_hits_non_filtered_sorted.bed -s -d 500 > results/tables/76595_E_mh/blaSHV_hits_non_filtered_merged.bed
```

```{r}
bla_merge <- readr::read_tsv("../results/tables/76595_E_mh/blaSHV_hits_non_filtered_merged.bed", col_names = FALSE, show_col_types = FALSE)
...
The mode is the same, maximum is not

The mean is same too.
...
## Filtering 3: by orientation of the repeat unit and FR red

This section should become a script and replace `filter_fr_hits.R``
...
How many rep units detected per read?
...
filter hits by length, e.value and identity
...
filtering by this distance would mean that we keep only those reads that have at least one repeat unit in proximity to the FR and in correct orientation

```{r}
red_repunit %>% 
  ggplot(aes(query.x, distance)) +
  geom_boxplot(alpha = 0.5) +
  xlab("")
```

```{r}
summary(red_repunit$distance)
```

Take the minimum distance for each read and plot its distribution

```{r}
red_repunit %>% 
  group_by(subject) %>% 
  mutate(min_dist = min(abs(distance))) %>% 
  select(query.x, subject, min_dist) %>% 
  distinct() %>% 
  ggplot(aes(min_dist)) +
  geom_histogram(bins=100) +
  geom_rug()
