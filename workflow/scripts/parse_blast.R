...
parse_blast_df <- function(df_name, minimal_length=250, filter_on_positive=TRUE){
  # read blast results
  df <- read_delim(df_name, col_names = FALSE, col_select = c(X1, X2, X3, X4, X9, X10))
  names(df) <- c("query", "subject", "identity", "length", "start.subject", "end.subject")
...
join_filtered_blast <- function(df1_path, df2_path, min_len, on_pos){
  parsed_list <- lapply(c(df1_path, df2_path), function(x){
    parse_blast_df(x, minimal_length=min_len, filter_on_positive=on_pos)
    })
...
fr12_pos <- join_filtered_blast(opt$input1, opt$input2, min_len=opt$hit_len, on_pos=TRUE)

# COLLECT NEG STRAND ROWS W BOTH FR
fr12_neg <- join_filtered_blast(opt$input1, opt$input2, min_len=opt$hit_len, on_pos=FALSE)
...
n_copies_counts <- n_copies %>% group_by(n.copies) %>% summarize(counts=n())

# WRITE THE TABLE W COUNTS
write.csv(n_copies_counts, opt$output_table, row.names = FALSE, quote = FALSE)
