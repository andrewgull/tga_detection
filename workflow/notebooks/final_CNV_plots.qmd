...
fr_ru_filt %>% select(subject, distance.btw.FR)
```

data on counts through BED files merging come from the rule `blaSHV_counts`

```{r}
bla_counts <- read_delim("../../results/tables/76595_D_10g/blaSHV_counts.tsv", show_col_types = F)

counts_distances <- fr_ru_filt %>% 
  select(subject, distance.btw.FR) %>% 
  left_join(bla_counts, by="subject") %>% 
  mutate("n.blaSHV.dist" = distance.btw.FR/860,
         "diff" = round(abs(n.blaSHV.dist - n.blaSHV.merged)))
