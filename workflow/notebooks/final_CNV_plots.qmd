...
where *Nbla1* - the number of bla hits as calculated by blasting bla (+merging bed files) and *Nbla2* - the number of bla genes as calculated by dividing the distance btw FR by one bla length

The idea is that the distance should always be bigger than the number of bla genes counted by merging the hits.

### Tables

data on distances between FR come from the rule `filter_flanking_regions`

Let's look at minimal difference between d(FR)/len(bla) and n(bla).
...
range_dist <- function(sample_name, bla_length=860){
  ...
  diff_per_read <- fr_ru_filt %>% 
    ...
    mutate("n.blaSHV.dist" = distance.btw.FR/bla_length,
           "diff" = round(n.blaSHV.dist - n.blaSHV.merged))
    out_dist <- tibble("sample" = sample_name,
                     "min.diff" = min(diff_per_read$diff, na.rm = TRUE),
                     "max.diff" = max(diff_per_read$diff, na.rm = TRUE))
  return(out_dist)
}
...
`min.diff` shows a minimal estimate of how many more *blaSHV* genes are expected to be found in a read based on the distance between FRs compared to the number of *blaSHV* genes calculated via BLAST analysis.

Negative numbers in `min.diff` is a sign of 'bad' reads.

```{r}
diff_bla_counts |> arrange(min.diff)
```

Now, let's look at the max differences.

```{r}
diff_bla_counts |> arrange(max.diff)
```
...
full_dist <- function(sample_name, bla_length=860){
  # this FR table contains distance between FRs
  fr_ru_filt <- read_delim(paste0("../../results/tables/", sample_name, "/blast_joined.tsv"), show_col_types = F, progress= F)
  # this one contains bla counts via merging BED coords
  bla_counts <- read_delim(paste0("../../results/tables/", sample_name, "/blaSHV_counts.tsv"), show_col_types = F, progress = F)
  ...
  diff_per_read <- fr_ru_filt %>% 
    ...
    mutate("sample" = sample_name,
           "n.blaSHV.dist" = distance.btw.FR/bla_length,
           "diff" = round(n.blaSHV.dist - n.blaSHV.merged)) %>% 
    select(-c(n.blaSHV.dist, distance.btw.FR)) %>% 
    na.omit()
...
```{r}
# let's try one sample with bigger range 76595_D_40g
full_sample_dist <- full_dist(sample_name = "76595_D_40g")

# plot
ggplot(full_sample_dist, aes(sample, diff)) +
  geom_boxplot() +
...
plot_list <- map(full_dist_list, ~ make_plots(.x))

ggarrange(plotlist = plot_list[1:16], nrow = 4, ncol = 4)
```

```{r, fig.height=8, fig.width=8}
ggarrange(plotlist = plot_list[17:32], nrow = 4, ncol = 4)
```

```{r, fig.height=8, fig.width=8}
ggarrange(plotlist = plot_list[33:48], nrow = 4, ncol = 4)
```

```{r, fig.height=8, fig.width=8}
ggarrange(plotlist = plot_list[49:64], nrow = 4, ncol = 4)
```
