...
# samples
sample_names <- dir("../../results/tables", pattern = "76595_*" )

aberrant_reads_df <- map_dfr(sample_names, ~ main(.))
...
D=mod(Nbla1 -Nbla2) where Nbla1 - the number of bla hits as calculated by blasting bla (+merging bed viles) and Nbla2 - the number of bla genes as calculated by dividing the distance btw FR by one bla length
...
mod_dist <- function(sample_name, bla_length=860){
  ...
  min_dist <- tibble("sample" = sample_name,
                     "min.diff" = min(diff_per_read$diff))

  return(min_dist)
}

diff_bla_counts <- map_dfr(sample_names, ~ mod_dist(.x))

write_delim(diff_bla_counts, "../../results/tables/min_diff_bla_counts.tsv", delim = "\t")
```

Look at the min differences in *blaSHV* counts
