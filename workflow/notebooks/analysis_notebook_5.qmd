...
Here, I will check how many of these bla SHV hits reside between the FR to exclude aberrant reads.
...
# 4 reads in this sample have at least one bla hit outside the FR range
...
If you need TRUE/FALSE value next to each read, the functions above have to be adjusted.

For one table it should look like this:

```{r}
fr_ru_filt$aberrant <- map_vec(fr_ru_filt$subject, ~ genes_within_flanks(., fr_ru_filt, bla_filt))

fr_ru_filt |> filter(aberrant) # teh same 4 reads are aberrant but now they are marked
```

### Counts of aberrant reads per sample (table)
